...
const BollywoodMovie = ({ searchTerm }) => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const bollywoodMovies = data.filter(movie => movie.category === 'Bollywood');
      ...
      <div className="mx-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">
        {currentMovies.map((movie) => (
          <div
            ...
            id={movie._id}
            className="bg-black rounded-xl shadow-md overflow-hidden border border-white" // Changed background color to light gray
          >
            <img
              className="h-48 w-full object-contain"
              src={`http://localhost:5000${movie.image}`} // Assuming image path is stored in the backend
              alt={movie.title}
            />
            <div className="p-4">
